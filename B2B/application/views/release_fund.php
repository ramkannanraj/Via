<div class="well">
    ...
    <form class="form" class="frmupdate" role="form" action="<?php echo site_url() ?>home/relase_update" method="POST">
    ...
    <?php foreach($querys->result() as $row):?>
					  ...
					  <?php	}else{
                            ...
                            <input type="text" name="name" readonly class="form-control" value="Released">
                          </div>         
                          <div class="form-group">
                            <label for="exampleInputEmail2">Released By</label>
                            <input class="form-control" name="companyname"   type="text" value="<?php $val=$this->session->userdata('type');echo $val;  ?>" >
                          </div>
                         <!-- <div class="form-group">
                            <label for="exampleInputPassword2">Released By ID</label>
                            <input type="text" class="form-control" readonly name="parent_id" value="<?php echo $row->parent_id?>">
                          </div>-->
                            
						   <div class="form-group">
                            <label for="exampleInputPassword2">Released To</label>
                            <input type="text" name="name" readonly class="form-control" value="<?php echo $row->name?>">
                          </div>
						  <!--<div class="form-group">
                            <label for="exampleInputPassword2">Released to ID</label>
						  ...
						  <input type="hidden" name="tot_bal" readonly class="form-control" value="<?php echo $row->total_balance?>">
                             <input type="hidden" name="used_balance" readonly class="form-control" value="<?php echo $row->used_balance?>">
						  <div class="form-group">
                            <label for="exampleInputPassword2">Released Amount</label>
                            <input type="text" name="amt"  class="form-control" value="">
                          </div>
						   <?php  /*$res_total_balz=$row->locked_balance;$vars='0';
...
$(document).ready(function (){
    $(".frmupdate").submit(function(e){
        e.preventDefault();
        $.ajax({
            url:'<?php echo site_url() ?>home/relase_update',
            type:'POST',
            dataType:'json',
